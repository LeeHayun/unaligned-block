...
def param_check(name, param, args):
    if (name.split('.')[-1] == "weight"
            and param.dim() == 4
            and param.size(0) >= args.min_oc
            and param.size(1) >= args.min_ic
            and param.size(2) == 1
            and param.size(3) == 1):
        return True
    else:
...
def update_Z(X, U, args):
    ...
    if args.sparsity_method == "gt":
        ...
        for x, u in zip(X, U):
            ...
            if args.unaligned and args.unaligned_score:
                ...
                accumulated_score, _ = calc_unaligned_greedy(z_np, GS=(args.block_size, 1), norm_policy=args.block_norm, min_sparsity=args.min_sparsity)
                if args.normlized_score:
                    score = (accumulated_score[1:] - accumulated_score[:-1]) / accumulated_score[1:]
                else:
                    score = accumulated_score[1:] - accumulated_score[:-1]
                score_list.append(score)
            else:
                if args.block_norm == 'l1':
                    m = z.reshape(co // args.block_size, args.block_size, ci, kh, kw).abs().sum(1)
                elif args.block_norm == 'l2':
    ...
    for x, u in zip(X, U):
        ...
        if args.unaligned:
            ...
            _, mask = calc_unaligned_greedy(z_np, GS=(args.block_size, 1), norm_policy=args.block_norm, target_M=target_num_block)
...
def prune_weight(weight, args, idx):
    ...
    if args.unaligned:
        ...
        if args.block_norm == 'l1':
            m = weight.reshape(co // args.block_size, args.block_size, ci, kh, kw).abs().sum(1).detach().cpu().numpy()
        elif args.block_norm == 'l2':
...
def prune_with_mask(model, dict_mask, args):
    for name, param in model.named_parameters():
        if param_check(name, param, args):
            mask = dict_mask[name]
            param.data.mul_(mask)
...
def print_convergence(model, X, Z, args):
    idx = 0
